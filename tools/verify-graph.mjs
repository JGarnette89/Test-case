...
import { onRightOf, oncoming, intentOf, graphOf, laneSpanOnGraph, ONCOMING_TOL } from "../src/sim/graph.js";
...
const crossroads = (reach = 300) => {
  const m = emptyMap("x");
  const c = { x: 500, y: 500 };
  m.roads.push(road({ id: "N", points: line({ x: c.x, y: c.y - reach }, c) }));
  m.roads.push(road({ id: "S", points: line({ x: c.x, y: c.y + reach }, c) }));
  m.roads.push(road({ id: "E", points: line({ x: c.x + reach, y: c.y }, c) }));
  m.roads.push(road({ id: "W", points: line({ x: c.x - reach, y: c.y }, c) }));
  return loadMap(m);
};
...
/* 2. EQUIVALENCE: a crossroads from four map strokes IS the compass crossroads. */
{
  ...
  const legOf = { N: "N|end", S: "S|end", E: "E|end", W: "W|end" };
...
{
  const m = emptyMap("t");
  m.roads.push(road({ id: "main", points: line({ x: 0, y: 300 }, { x: 600, y: 300 }, 60) }));
  m.roads.push(road({ id: "side", points: line({ x: 300, y: 600 }, { x: 300, y: 300 }), control: { start: "none", end: "stop" } }));
  const t = loadMap(m);
  const g = graphOf(t);
  ...
  for (const [id, deg] of [["a", 0], ["b", 45], ["c", 135], ["d", 180], ["e", 270]]) {
    ...
    five.roads.push(road({ id, points: line(far, c), control: { start: "none", end: "stop" } }));
  }
...
/* 5. The test map: a loop through four nodes joined by a bend and a hill, and an overpass. */
{
  ...
  const g = graphOf(l);
  check(g.links.length === 4 && g.at.filter((s) => s.through).length === 2, `four roads join nodes to nodes (the loop), and the overpass is a lane with nothing on it, both ways (${g.links.length} links, ${g.at.filter((s) => s.through).length} through lanes)`);
  ...
  check(spawnedAtEdges && edgesOf(two.w.course).length === 9, `traffic enters only at dangling ends: ${edgesOf(two.w.course).length} of them (seven roads to the edge, the overpass both ways)`);
}
...
/* 6. Stage 0's map through the graph: four lanes with nothing on them, driven end to end. */
{
  ...
  const g = graphOf(l);
  check(g.at.length === 4 && g.at.every((s) => s.through) && g.links.length === 0, "stage 0 is four through lanes and no intersection");
